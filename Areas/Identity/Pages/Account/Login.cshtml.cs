    ...
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //kullanıcı login oldugunda sepetteki urunlerini gosteriyoruz.logout post metodundada sessionu 0 yapıyoruz.
                    var user = await _db.Users.Where(u => u.Email == Input.Email).FirstOrDefaultAsync();

                    List<ShoppingCart> listShoppingCart = await _db.ShoppingCart.Where(x => x.ApplicationUserId == user.Id).ToListAsync();
